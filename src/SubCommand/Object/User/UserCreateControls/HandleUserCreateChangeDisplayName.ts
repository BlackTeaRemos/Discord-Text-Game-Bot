...
export async function HandleUserCreateChangeDisplayName(
    ...
    try {
        const newDisplayName = await PromptText({
            ...
            prompt: `Send the display name you want to use. Type **cancel** to keep the current value.`,
            minLength: 1,
            maxLength: 100,
            cancelWords: [`cancel`],
        });
        ...
        const normalized = NormalizeUserCreatePromptErrorMessage({
            message,
            cancelMessage: UserCreatePromptMessages.displayNameCancel,
            timeoutMessage: UserCreatePromptMessages.displayNameTimeout,
